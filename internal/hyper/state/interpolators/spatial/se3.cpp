...
auto SpatialInterpolator<SE3<TScalar>, Tangent<SE3<TScalar>>>::evaluate(const Index& derivative, const Scalar* const* inputs, const Index& num_inputs, const Index& start_index,
  ...
  if (!jacobians) {
    // Retrieves first input.
    const auto T_0 = I(start_index).gExp();
    ...
    for (Index i = end_index; start_index < i; --i) {
      ...
      const auto R_ab = T_a.angular().gExp().gInv().gPlus(T_b.angular().gExp());
      const auto d_ab = R_ab.gLog();
      const auto x_ab = Translation{T_b.linear() - T_a.linear()};
      const auto w_ab = RotationTangent{w0_i * d_ab};
      const auto R_i = w_ab.gExp();
      const auto x_i = Translation{w0_i * x_ab};
    ...
    // Jacobian lambda definitions.
    auto Jr = [&result, &input_offset](const Index& k, const Index& i) {
      return result.template jacobian<Input::Angular::kNumParameters, RotationTangent::kNumParameters>(k, i, Input::kAngularOffset, Input::kAngularOffset + input_offset);
    };

    auto Jt = [&result, &input_offset](const Index& k, const Index& i) {
      return result.template jacobian<Input::Linear::kNumParameters, Translation::kNumParameters>(k, i, Input::kLinearOffset, Input::kLinearOffset + input_offset);
    };
    ...
    const auto T_a = I(start_index).gExp();
    R = T_a.rotation() * R;
    x = T_a.translation() + x;
  }
