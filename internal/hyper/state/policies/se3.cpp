...
auto evaluate(const StateQuery& state_query, const PolicyQuery& policy_query) -> StateResult {
  ...
  DCHECK(num_inputs == layout.outer.size() + 1 && weights.rows() == layout.inner.size() + 1 && weights.cols() == TDerivative + 1);
  ...
  // Compute indices.
  const auto start_idx = layout.inner.lowerBound();
  const auto end_idx = layout.inner.upperBound();
